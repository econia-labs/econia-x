...
slippage price $p_s$ after the swap. As derived in
...
identical to finding the roots of Equation~\ref{eqn:swap-buy-with-fee-slippage-input}, a quadratic polynomial in $q_s$ containing the common term
$T = [(1 - f) \cdot b_i \cdot p_s]$.

\begin{equation}\label{eqn:swap-buy-with-fee-slippage-input}
	\begin{aligned}
		F(b_i, f, p_s, q_i, q_s) & =
		q_s^2 + q_s \cdot (2 \cdot q_i - f \cdot T) + (q_i^2 - q_i \cdot T), \\
		T                        & = [(1 - f) \cdot b_i \cdot p_s]
	\end{aligned}
\end{equation}
