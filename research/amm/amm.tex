...
In practice, Equation~\ref{eqn:buy-with-fee-quadratic} requires considerable
intermediate computation and range checking to prevent errors like overflow.
Alternatively, for a maximum quote input amount $q_{\max}$ where $q_{in} = q_{\max}$
yields $p_s > p_{ask}$, $q_s$ can thus be solved via Newton-Raphson convergence with
...
\begin{equation}\label{eqn:buy-with-fee-convergence}
	\begin{aligned}
		...
		q_{n + 1}                     & =
		\frac{q_n^2 + q_i \cdot t - q_i^2}{2 \cdot q_n + 2 \cdot q_i - f \cdot t}, \\
		q_0                           & = q_{\max}
	\end{aligned}
\end{equation}
...
for $q_0$, derived in Section~\ref{sssec:equation-eqn-buy-with-fee-q-0-derivation}.

\begin{equation}\label{eqn:buy-with-fee-q_0}
	q_0 = \frac{f \cdot t}{2}
\end{equation}
...
\input{derivations/buy-with-fee/q_0.tex}
