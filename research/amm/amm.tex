...
As derived in Section~\ref{sssec:equation-eqn-swap-buy-with-fee-out-derivation},
...
slippage price $p_s$ after the swap. Equation~\ref{eqn:swap-buy-with-fee-slippage-price}
does afford a closed-form solution for $q_s$ in terms of the other variables including
$p_s$, but it is non-trivial and due to its numerical implementation complexity it is
not provided here.

Rather, consider that in practice $p_s$ can be represented as the ratio
$\frac{p_{s, q}}{p_{s, b}}$, such that
Equation~\ref{eqn:swap-buy-with-fee-slippage-price} can be alternatively represented as
Equation~\ref{eqn:swap-buy-with-fee-slippage-price-ratio}.

\begin{equation}\label{eqn:swap-buy-with-fee-slippage-price-ratio}
  p_{s, q} \cdot (1 - f) \cdot b_i \cdot (q_i + f \cdot q_{in}) =
  p_{s, b} \cdot {(q_i + q_{in})}^2
\end{equation}
