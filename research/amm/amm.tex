...
Note the following two relationships between $t$ and $q_i$, derived respectively in
Sections~\ref{sssec:equation-eqn-buy-with-fee-t-eq-q-i-derivation}
and~\ref{sssec:equation-eqn-buy-with-fee-t-gt-q-i-derivation}.

\begin{equation}\label{eqn:buy-with-fee-t-eq-q_i}
	p_{ask} = p_m \implies t = q_i
\end{equation}

\begin{equation}\label{eqn:buy-with-fee-t-gt-q_i}
	p_{ask} > p_m \implies t > q_i
\end{equation}
...
roots for $p_{ask} > p_m$, one positive and one negative. Thus $q_s$ can be solved
exactly using the quadratic formula, derived in
Section~\ref{sssec:equation-eqn-buy-with-fee-quadratic-derivation}.

\begin{equation}\label{eqn:buy-with-fee-quadratic}
	\begin{aligned}
		p_{ask} > p_m & \implies                          \\
...
In practice, Equation~\ref{eqn:buy-with-fee-quadratic} requires an iterative square root
computational algorithm like Heron's method for the intermediate square root term.
Alternatively, $q_s$ can be solved directly using Newton-Raphson convergence, derived in
Section~\ref{sssec:equation-eqn-buy-with-fee-convergence-derivation}.

\begin{equation}\label{eqn:buy-with-fee-convergence}
	q_{s, n + 1} = \frac{q_{s, n}^2 + q_i \cdot t - q_i^2}
	{2 \cdot q_{s, n} + 2 \cdot q_i - f \cdot t}
\end{equation}

Note that $q_s$ is only required in practice if the maximum possible quote input amount
$q_{\max}$ yields $p_s > p_{ask}$. If this condition is met, then $q_{\max}$ can be used
...
\begin{equation}\label{eqn:buy-with-fee-convergence-q_s_0-eq-q_max}
	p_s(q_{\max}) > p_{ask} > p_m \implies q_{s, 0} = q_{\max}
\end{equation}

The solution may still converge slowly when $q_{\max}$ is sufficiently large though, for
example when a taker fills a large order against substantial order book ask liquidity
but first needs to marginally slip the \gls*{cpamm}. Thus an approximation of
Equation~\ref{eqn:buy-with-fee-quadratic} can instead be used for $q_{s, 0}$ in the more
general case of $p_{ask} > p_{m}$, derived in
Section~\ref{sssec:equation-eqn-buy-with-fee-q-s-0-eq-f-t-derivation}.

\begin{equation}\label{eqn:buy-with-fee-q_s_0-eq-f_t}
	p_{ask} > p_m \implies q_{s, 0} = f \cdot t
\end{equation}

Combining the above yields the numerical solution for $q_s$, derived in
Section~\ref{sssec:equation-eqn-buy-with-fee-algorithm-derivation}.

\begin{equation}\label{eqn:buy-with-fee-algorithm}
	\begin{aligned}
		p_{ask} > p_m & \implies                                          \\
		q_{s, 0}      & = a                                               \\
		q_{s, 1}      & = \frac{a^2 + b}{a + c}                           \\
		q_{s, n + 1}  & = \frac{q_{s, n}^2 + b}{2 \cdot q_{s, n} + c - a} \\
		q_{s, n + 1}  & = q_{s, n} \implies q_s = q_{s, n}
	\end{aligned}
	\begin{cases}
		a = f \cdot t                           \\
		b = q_i \cdot t - q_i ^ 2               \\
...
\input{derivations/buy-with-fee/t-eq-q_i.tex}
\input{derivations/buy-with-fee/t-gt-q_i.tex}
\input{derivations/buy-with-fee/polynomial-inequality.tex}
\input{derivations/buy-with-fee/quadratic.tex}
\input{derivations/buy-with-fee/convergence.tex}
\input{derivations/buy-with-fee/q_s_0-eq-f_t.tex}
\input{derivations/buy-with-fee/algorithm.tex}
