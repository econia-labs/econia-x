...
computational algorithm like Heron's method for the square root term. Alternatively, for
a maximum quote input amount $q_{\max}$ where $q_{in} = q_{\max}$ yields
$p_s > p_{ask}$, $q_s$ can thus be solved via general Newton-Raphson convergence with
$q_0 = q_{\max}$ for the first iteration, derived in
Section~\ref{sssec:equation-eqn-buy-with-fee-convergence-derivation}.

\begin{equation}\label{eqn:buy-with-fee-convergence}
	\begin{aligned}
		p_s(q_{\max}) > p_{ask} > p_m & \implies                                  \\
...
Note that Equation~\ref{eqn:buy-with-fee-convergence} may still converge slowly when
$q_{\max}$ is sufficiently large, for example when a taker is filling a large order
against substantial order book ask liquidity but first needs to slip the \gls*{cpamm}.
Thus an approximation of Equation~\ref{eqn:buy-with-fee-quadratic} can instead be used
for $q_{s, 0}$, derived in
Section~\ref{sssec:equation-eqn-buy-with-fee-q-s-0-derivation}.

\begin{equation}\label{eqn:buy-with-fee-q_s_0}
	q_{s, 0} = \frac{f \cdot t}{2}
\end{equation}

Combining the above yields a hybrid numerical solution for $q_s$, derived in
...
\begin{equation}\label{eqn:buy-with-fee-algorithm}
	\begin{aligned}
		p_{ask} > p_m & \implies                                          \\
		q_{s, 0}      & = a                                               \\
		q_{s, 1}      & = \frac{q_{s, 0}^2 + b}{c}                        \\
		q_{s, n + 1}  & = \frac{q_{s, n}^2 + b}{2 \cdot q_{s, n} + c - d} \\
...
\input{derivations/buy-with-fee/q_s_0.tex}
