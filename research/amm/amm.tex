...
The final quote amount $q_f$ is the sum of the initial quote amount $q_i$ and the
quote input amount $q_{in}$, per Equation~\ref{eqn:swap-buy-with-fee-quote-final}.

\begin{equation}\label{eqn:swap-buy-with-fee-quote-final}
	q_f (q_i, q_{in}) = q_i + q_{in}
\end{equation}

As derived in Section~\ref{sssec:equation-eqn-swap-buy-with-fee-base-volume-derivation},
$b_v$ is a function of the initial base amount $b_i$, $q_i$, and $q_{in}$, per
Equation~\ref{eqn:swap-buy-with-fee-base-volume}.

\begin{equation}\label{eqn:swap-buy-with-fee-base-volume}
	b_v (b_i, q_i, q_{in}) = \frac{b_i \cdot q_{in}}{q_i + q_{in}}
\end{equation}

As derived in Section~\ref{sssec:equation-eqn-swap-buy-with-fee-base-out-derivation},
$b_{out}$ is given in equation Equation~\ref{eqn:swap-buy-with-fee-base-out}.

\begin{equation}\label{eqn:swap-buy-with-fee-base-out}
	b_{out} (b_i, f, q_i, q_{in}) = f^{\dagger} \cdot b_v (b_i, q_i, q_{in})
\end{equation}

The final base reserve amount $b_f$ is the difference of $b_i$ and $b_{out}$, per
Equation~\ref{eqn:swap-buy-with-fee-base-final}.

\begin{equation}\label{eqn:swap-buy-with-fee-base-final}
	b_f (b_i, f, q_i, q_{in}) = b_i - b_{out} (b_i, f, q_i, q_{in})
\end{equation}

The average taker execution price $p_t$ is the ratio of the quote input amount $q_{in}$
to the base output amount $b_{out}$, per
Equation~\ref{eqn:swap-buy-with-fee-taker-execution-price}.

\begin{equation}\label{eqn:swap-buy-with-fee-taker-execution-price}
	p_t (b_i, f, q_i, q_{in}) = \frac{q_{in}}{b_{out} (b_i, f, q_i, q_{in})}
\end{equation}

Define the marginal taker execution price $p_m$ as the taker execution price for a swap
of infinitesimal size. As derived in
Section~\ref{sssec:equation-eqn-swap-buy-with-fee-marginal-price-derivation},
$p_m (b_i, f, q_i)$ is given in Equation~\ref{eqn:swap-buy-with-fee-marginal-price}.

\begin{equation}\label{eqn:swap-buy-with-fee-marginal-price}
	p_m (b_i, f, q_i) = \frac{q_i}{f^{\dagger} \cdot b_i}
\end{equation}

Define the slippage price $p_s$ as $p_m$ after $q_{in}$ has been swapped, the point at
which additional quote will fill at a worse price compared with matching against an
order book best ask price that is also at $p_s$. As derived in
Section~\ref{sssec:equation-eqn-swap-buy-with-fee-slippage-price-derivation},
$p_s (b_i, f, q_i, q_{in})$ is given in
Equation~\ref{eqn:swap-buy-with-fee-slippage-price}.

\begin{equation}\label{eqn:swap-buy-with-fee-slippage-price}
	p_s (b_i, f, q_i, q_{in}) = \frac{q_f (q_i, q_{in})}
	{f^{\dagger} \cdot b_f (b_i, f, q_i, q_{in})}
\end{equation}

Define the slippage quote input amount $q_s$ as $q_{in}$ resulting in a specified
slippage price $p_s$ after a swap. As derived in
Section~\ref{sssec:equation-eqn-swap-buy-with-fee-slippage-input-derivation},
solving Equation~\ref{eqn:swap-buy-with-fee-slippage-price} for $q_s = q_{in}$ is
identical to finding the roots of Equation~\ref{eqn:swap-buy-with-fee-slippage-input},
a quadratic polynomial in $q_s$ containing the common term
$Q = (f^{\dagger} \cdot b_i \cdot p_s)$.

\begin{equation}\label{eqn:swap-buy-with-fee-slippage-input}
	F(b_i, f, p_s, q_i, q_s) = q_s^2 + B \cdot q_s + C,
	\begin{cases}
		B = 2 \cdot q_i - f \cdot Q, \\
		C = q_i^2 - q_i \cdot Q,     \\
		Q = f^{\dagger} \cdot b_i \cdot p_s
	\end{cases}
\end{equation}
...
As derived in
Section~\ref{sssec:equation-eqn-swap-buy-with-fee-quadratic-inequality-derivation},
when $p_s$ is greater than $p_m$, the quadratic polynomial $F$ is negative at $q_s = 0$,
per Equation~\ref{eqn:swap-buy-with-fee-quadratic-inequality}.

\begin{equation}\label{eqn:swap-buy-with-fee-quadratic-inequality}
	p_s > p_m \implies F(q_s = 0) < 0
\end{equation}

The second order derivative of $F$ with respect to $q_s$, $F''(q_s)$, is given in
Equation~\ref{eqn:swap-buy-with-fee-quadratic-double-prime}, indicating that $F$ is
concave up.

\begin{equation}\label{eqn:swap-buy-with-fee-quadratic-double-prime}
	F''(q_s) = \frac{\mathrm{d}^2 F}{\mathrm{d}q_s^2} = 2 > 0
\end{equation}

Equations~\ref{eqn:swap-buy-with-fee-quadratic-inequality}
and~\ref{eqn:swap-buy-with-fee-quadratic-double-prime} imply that $F(q_s)$ has two real
roots for $p_s > p_m$, one positive and one negative. The derivative of $F$ with respect
to $q_s$, $F'(q_s)$, is given in
Equation~\ref{eqn:swap-buy-with-fee-quadratic-prime}.

\begin{equation}\label{eqn:swap-buy-with-fee-quadratic-prime}
	F'(q_s) = \frac{\mathrm{d} F}{\mathrm{d} q_s} = 2 \cdot q_s + B
\end{equation}

As derived in
Section~\ref{sssec:equation-eqn-swap-buy-with-fee-convergence-derivation}, $q_s$ can
thus be solved via Newton-Raphson convergence with one update step per
Equation~\ref{eqn:swap-buy-with-fee-convergence}, for a maximum quote input amount
$q_{\max}$ when a swap of size $q_{\max}$ yields $p_s$ greater than the best
ask price $p_{ask}$.

\begin{equation}\label{eqn:swap-buy-with-fee-convergence}
	p_s(q_{\max}) > p_{ask} \implies
	q_s = \frac{q_{\max} \cdot Q (p_{ask})}
	{2 \cdot q_{\max} + 2 \cdot q_i - f \cdot Q (p_{ask})}
\end{equation}
...
\input{derivations/swap-buy-with-fee/base-volume.tex}
\input{derivations/swap-buy-with-fee/base-out.tex}
\input{derivations/swap-buy-with-fee/marginal-price.tex}
\input{derivations/swap-buy-with-fee/slippage-price.tex}
\input{derivations/swap-buy-with-fee/slippage-input.tex}
\input{derivations/swap-buy-with-fee/quadratic-inequality.tex}
\input{derivations/swap-buy-with-fee/convergence.tex}
